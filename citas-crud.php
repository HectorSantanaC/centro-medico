 ...
 *       Usa include si no quieres que sea obligatorio.
 */
require_once 'config/Database.php';
...
if ($action === 'list') {
    ...
    /**
     * SQL con JOINs múltiples:
     * 
     * FROM citas c
     *   → Tabla principal: citas (alias 'c')
     * LEFT JOIN usuarios u ON c.paciente_id = u.id
     *   → Unir tabla usuarios (alias 'u') para obtener nombre del paciente
     * LEFT JOIN medicos m ON c.medico_id = m.id
     *   → Unir tabla medicos (alias 'm') para obtener nombre del médico
     * LEFT JOIN especialidades e ON c.especialidad_id = e.id
     *   → Unir tabla especialidades (alias 'e') para obtener nombre
     * 
     * ¿Por qué LEFT JOIN?
     *   LEFT JOIN devuelve TODAS las filas de la tabla izquierda (citas)
     *   aunque no haya coincidencia en la tabla derecha.
     *   INNER JOIN solo devolvería filas donde hay coincidencia en AMBAS.
     */
    $citas = $db->fetchAll(
        "SELECT c.*, 
     ...
     *   - DESC = De mayor a menor (primero los más nuevos)
     *   - ASC = De menor a mayor (primero los más antiguos)
     * 
     * Ejemplo de fecha DESC:
     *   2024-06-15, 2024-06-10, 2024-06-01
     */
}
...
// ============================================================
/**
 * A partir de aquí, el código PHP genera HTML mezclado con PHP.
 * El servidor ejecuta el PHP, sustituye las variables,
